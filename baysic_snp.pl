...
my %opt = ();
my $results = GetOptions (\%opt,'fasta|f=s','capture|b=s','help|h','prefix|p=s','complex|c=s','execdir|e=s');
...
#bedtools multiinter -header -i gatk1.snp.vcf.gz platypus1.snp.vcf.gz sam1.snp.vcf.gz ssvar1.snp.vcf.gz > snp_intersect.txt

open VC, "<snp_intersect.txt" or die $!;
my $header = <VC>;
chomp($header);
